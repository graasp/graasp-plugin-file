...
describe('Plugin Tests', () => {
  ...
  describe('GET /thumbnails/:id', () => {
    it('Successfully download all different THUMBNAIL_SIZES', async () => {
    ...
    it('Successfully download all different THUMBNAIL_SIZES with storage prefix', async () => {
      const app = await build({
        taskManager,
        runner,
        membership,
  ...
  describe('POST /thumbnails/:id', () => {
    ...
    it('Successfully upload thumbnail', async () => {
      const put = jest.spyOn(S3Provider.prototype, 'uploadFile');
      const app = await build({
        taskManager,
        runner,
        membership,
      ...
      expect(put).toBeCalledTimes(SIZES.length);
    });
  });
});
